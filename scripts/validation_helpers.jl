# Functions for writing out the regression-test validation data.
#
# These are the write-side counterparts of the `validate_*` functions in
# `test/utils.jl`. They live here rather than in the test suite because nothing
# in `test/` calls them -- only `scripts/save_validation_data.jl` does.
#
# Keep these in sync with `test/utils.jl`: the layout written here is exactly
# what the `validate_*` functions expect to read back.
