...
@defcomp DamageAggregator_NewSectorDamages begin
    ...
    function run_timestep(p, v, d, t)
        ...
        v.total_damage[t] =
            (p.include_cromar_mortality ? v.cromar_mortality_damage[t] : 0.0) +
            (p.include_ag ? v.agriculture_damage[t] : 0.0) +
            (p.include_energy ? v.energy_damage[t] : 0.0) +
            (p.include_new_sector ? v.new_sector_damage[t] : 0.0) +
            (p.include_dice2016R2 ? p.damage_dice2016R2[t] * 1e9 : 0.0) +
            (p.include_hs_damage ? p.damage_hs[t] * 1e9 : 0.0)
        ...
        v.total_damage_domestic[t] =
            (p.include_cromar_mortality ? v.cromar_mortality_damage_domestic[t] : 0.0) +
            (p.include_ag ? v.agriculture_damage_domestic[t] : 0.0) +
            (p.include_energy ? v.energy_damage_domestic[t] : 0.0) +
            (p.include_new_sector ? v.new_sector_damage_domestic[t] : 0.0)
    end
end
