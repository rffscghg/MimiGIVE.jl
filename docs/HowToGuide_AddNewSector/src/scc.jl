...
function modified_post_trial_func(mcs::SimulationInstance, trialnum::Int, ntimesteps::Int, tup)
    ...
    slr_mds = include_slr ? ciam_mds.globe : fill(0.0, length(MimiGIVE._model_years))
    total_mds = main_mds .+ slr_mds

    if options.compute_domestic_values
        main_mds_domestic = (damages_marginal_domestic .- damages_base_domestic) .* gas_units_multiplier
        slr_mds_domestic = include_slr ? ciam_mds.domestic : fill(0.0, length(MimiGIVE._model_years))
    ...
    for dr in discount_rates
        ...
        if options.certainty_equivalent
            df_ce = [((1.0 / cpc[i])^dr.eta * 1 / (1 + dr.prtp)^(t - year) for (i, t) in enumerate(MimiGIVE._model_years) if year <= t <= last_year)...] # only used if optionas.certainty_equivalent=true
        end
