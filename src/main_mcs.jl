...
function get_mcs(trials;
    ...
    if fair_parameter_set == :deterministic
        ...
        sum(fair_parameter_set_ids .> 2237) > 0.0 || sum(fair_parameter_set_ids .< 1) > 0.0 && error("FAIR parameter set ids must be between 1 and 2237, inclusive.")
    end
    ...
    if rffsp_sampling == :deterministic
        ...
        sum(rffsp_sampling_ids .> 10_000) > 0.0 || sum(rffsp_sampling_ids .< 1) > 0.0 && error("RFF SP sample ids must be between 1 and 10,000, inclusive.")
    end
    ...
    hs_σ_4 = [0.0362838946808348 -0.0420628550865489 0.0
        -0.0420628550865489 0.0714335834791260 0.0
        0.0 0.0 0.0157459807497214]
    ...
    hs_σ_8 = [0.0104404075456397 -0.0104767876031064 0.0
        -0.0104767876031064 0.010634289819357 0.0
        0.0 0.0 0.0563560833680617]
...
function get_fair_mcs_params(n::Int;
    ...
    if fair_parameter_set == :deterministic
        ...
        sum(fair_parameter_set_ids .> 2237) .> 0 || sum(fair_parameter_set_ids .< 1) > 0.0 && error("FAIR parameter set ids must be between 1 and 2237, inclusive.")
    end
