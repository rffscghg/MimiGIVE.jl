...
const scc_gas_molecular_conversions = Dict(:CO2 => 12 / 44, # C to CO2
    :N2O => 28 / 44, # N2 to N2O,
    :CH4 => 1.0, # CH4 to CH4
    :HFC23 => 1.0, # HFC23 to HFC23
    :HFC32 => 1.0, # HFC32 to HFC32
    :HFC43_10 => 1.0, # HFC43_10 to HFC43_10
    :HFC125 => 1.0, # HFC125 to HFC125
    :HFC134a => 1.0, # HFC134a to HFC134a
    :HFC143a => 1.0, # HFC143a to HFC143a
    :HFC227ea => 1.0, # HFC227ea to HFC227ea
    :HFC245fa => 1.0) # HFC245fa to HFC245fa
...
function _compute_scc(mm::MarginalModel;
    ...
    if domestic
        ...
        ciam_marginal_damages = mm.base[:DamageAggregator, :include_slr] ? _compute_ciam_marginal_damages(mm.base, mm.modified, gas, ciam_base, ciam_modified, segment_fingerprints; CIAM_foresight=CIAM_foresight, CIAM_GDPcap=CIAM_GDPcap, pulse_size=pulse_size).domestic : fill(0.0, length(_model_years))
    else
        main_marginal_damages = mm[:DamageAggregator, :total_damage] .* scc_gas_molecular_conversions[gas]
        ciam_marginal_damages = mm.base[:DamageAggregator, :include_slr] ? _compute_ciam_marginal_damages(mm.base, mm.modified, gas, ciam_base, ciam_modified, segment_fingerprints; CIAM_foresight=CIAM_foresight, CIAM_GDPcap=CIAM_GDPcap, pulse_size=pulse_size).globe : fill(0.0, length(_model_years))
    end
...
function post_trial_func(mcs::SimulationInstance, trialnum::Int, ntimesteps::Int, tup)
    ...
    if include_slr
        ...
        ciam_mds.globe[1:year_index] .= 0.0
        ciam_mds.domestic[1:year_index] .= 0.0
    end

    main_mds = (damages_marginal .- damages_base) .* gas_units_multiplier
    slr_mds = include_slr ? ciam_mds.globe : fill(0.0, length(_model_years))
    total_mds = main_mds .+ slr_mds

    if options.compute_domestic_values
        main_mds_domestic = (damages_marginal_domestic .- damages_base_domestic) .* gas_units_multiplier
        slr_mds_domestic = include_slr ? ciam_mds.domestic : fill(0.0, length(_model_years))
    ...
    if options.save_slr_damages
        ...
        if include_slr
            ...
            # global
            slr_damages[:base][trialnum, :] .= 0.0
            slr_damages[:modified][trialnum, :] .= 0.0
            slr_damages[:base_lim_cnt][trialnum, :, :] .= 0.0
            slr_damages[:modified_lim_cnt][trialnum, :, :] .= 0.0
            slr_damages[:base_segments_2100][trialnum, :] .= 0.0

            # domestic - these Dictionary entries will only exist if we are computing
            # domestic values
            if options.compute_domestic_values
                slr_damages[:base_domestic][trialnum, :] .= 0.0
                slr_damages[:modified_domestic][trialnum, :] .= 0.0
            end
        end
    end
    ...
    for dr in discount_rates
        ...
        if options.certainty_equivalent
            df_ce = [((1.0 / cpc[i])^dr.eta * 1 / (1 + dr.prtp)^(t - year) for (i, t) in enumerate(_model_years) if year <= t <= last_year)...] # only used if options.certainty_equivalent=true
        end
...
function _compute_ciam_marginal_damages(base, modified, gas, ciam_base, ciam_modified, segment_fingerprints; CIAM_foresight, CIAM_GDPcap, pulse_size)
    ...
    if CIAM_GDPcap
        ...
        base_lim_cnt = fill(0.0, length(_damages_years), num_ciam_countries)
        modified_lim_cnt = fill(0.0, length(_damages_years), num_ciam_countries)
    end
    ...
    return (globe=[fill(0.0, 2020 - _model_years[1]); damages_marginal], # billion USD $2005
        domestic=[fill(0.0, 2020 - _model_years[1]); damages_marginal_domestic], # billion USD $2005
        damages_base=[fill(0.0, 2020 - _model_years[1]); damages_base], # billion USD $2005
        damages_modified=[fill(0.0, 2020 - _model_years[1]); damages_modified], # billion USD $2005
        damages_base_domestic=[fill(0.0, 2020 - _model_years[1]); damages_base_domestic], # billion USD $2005
        damages_modified_domestic=[fill(0.0, 2020 - _model_years[1]); damages_modified_domestic], # billion USD $2005
